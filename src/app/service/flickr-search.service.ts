...
export class FlickrService {
  ...
  getResult(query: string) {
    let url = `https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=${this.key}&tags=${query}&per_page=30&format=json&nojsoncallback=1`;
    return this._http
      .get(url)
      .map(res => res.json())
      .map((data) => {
  ...
  getExif(photoId: string) {
    let url = `https://api.flickr.com/services/rest/?method=flickr.photos.getExif&api_key=${this.key}&photo_id=${photoId}&format=json&nojsoncallback=1`;
    return this._http
      .get(url)
      .map(res => res.json())
      .map((data) => {
